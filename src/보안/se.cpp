...
void InitialPermutation(DESED * Data)
{
    ...
    for (int i = 31; i >= 0; i--)
    {
        DESED tmpMask = 0x00000001;
        DESED setMask = 0x00000001 << (31 - i);
...
void InverseInitialPermutation(DESED * Data)
{
    /* 입력 된 64Bit를 32Bit 씩 나누는 배열 변수 */
    DESED NewData[2] = { 0, 0 };
    /* NewData[1]에 31번째 인덱스에서 0번째 인덱스까지 데이터 저장 (32Bit) */
    for (int i = 31; i >= 0; i--)
    {
        DESED tmpMask = 0x00000001; /* 0x00000001 = 1 */
        DESED setMask = 0x00000001 << (31 - i);
...
void ExpansionPermutation(DESED RightHalf, DESED * eWord)
{
...
DESED PermutationFunction(DESED dWord)
{
    DESED tmpMask = 0x00000001;
    DESED keyMask = 0x00000001;
    DESED dwTmp = 0;
    for (int i = 0; i < 32; i++)
    {
...
DESED SubstitutionChoice(DESED * eWord)
{
...
void Round(short mSwitch, DESED * dWord, DESED RoundKey1, DESED RoundKey0)
{
    DESED tmp[2] = { 0,0 };
...
void PermutedChoice1(DESED * Key)
{
    DESED tmpMask;
    DESED KeyMask;
    DESED NewKey[2] = { 0,0 };
...
void PermutedChoice2(DESED *Key)
{
    DESED tmpMask;
    DESED KeyMask;
    DESED NewKey[2] = { 0,0 };
...
void KeySchedule(const char * Key, DESED * RoundKey)
{
    int SHIFT = 0;
    DESED dwKey[2] = { 0,0 };
    DESED tmp0, tmp1;
    ...
    {
        SHIFT = ((3 - i) * 8);
        dwKey[1] ^= ((DESED)Key[i]) << SHIFT;
        dwKey[0] ^= ((DESED)Key[i + 4]) << SHIFT;
    }
...
/* En/De cryption 함수 */
void Cryption_EnDe(short mSwitch, DESED * data, DESED * RoundKey)
{
